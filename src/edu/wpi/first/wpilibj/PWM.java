...
public class PWM extends SensorBase implements LiveWindowSendable {
    ...
    private void initPWM(final int channel) {
        ...
        System.out.println("trying to init pwm " + channel);
        if (PWMStore.exists(channel)) {
            throw new RuntimeException("There is already a PWM assigned on channel " + channel);
        } else {
            PWMStore.initPWM(channel, this);
        }
    }
    ...
    protected void setBounds(double max, double deadbandMax, double center, double deadbandMin, double min) {
        ...
        // Picked a random number that makes this work ok.
        double loopTime = .0005;
