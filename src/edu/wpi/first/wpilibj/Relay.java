...
public class Relay extends SensorBase implements LiveWindowSendable {
    ...
    private Value m_cur_val = Value.kOff;
    ...
    public Relay(final int channel, Direction direction) {
        ...
        initRelay();
        set(Value.kOff);
        if (RelayStore.exists(m_channel)) {
            throw new RuntimeException("There is already a Relay assigned on channel " + m_channel);
        } else {
            RelayStore.initRelay(m_channel, this);
        }
    }
